...
(function (dt) {
    ...
    var RepositoryList = (function (_super) {
            ...
            animateHeight($item, function () {
                if(!$item.hasClass('expanded')) {
                    ...
                    if($item.find('.details').length == 0) {
                        ...
                        var content = _(_this.list).findWhere({
                            id: id
                        });
                        if(content) {
        ...
        RepositoryList.prototype.renderPagination = function () {
            ...
            this.$pagination.empty().toggleClass('empty', this.numPages < 2);
            if(mid > 0) {
                this.$pagination.append(this.paginationTemplate({
                    index: mid - 1,
                    display: '&lt;'
                }));
            }
