...
(function (dt) {
    var Application = (function () {
        function Application() {
            ...
            if (search && list) {
                new dt.RepositorySearch(search, new dt.RepositoryList(list));
            }
        }
        return Application;
    })();
...
(function (dt) {
    ...
    dt.Repository = Repository;

    var RepositorySearch = (function () {
    ...
    dt.RepositorySearch = RepositorySearch;
    ...
    var RepositoryList = (function () {
        ...
        RepositoryList.prototype.toggle = function (el, expand) {
            ...
            dt.animateHeight($item, function () {
    ...
    dt.RepositoryList = RepositoryList;

    dt.repository = new Repository('/tsd/data/repository.json');
})(dt || (dt = {}));
var dt;
(function (dt) {
    dt.transition;
    ...
    function animateHeight($el, callback, success) {
        ...
        noTransition($el, function () {
            ...
            $el.css('height', '');
            to = $el.height();
            if (from != to && dt.transition)
                $el.css('height', from);
        });

        if (from != to && dt.transition) {
            $el.css('height', to);
            $el.on(dt.transition.endEvent, function () {
                noTransition($el, function () {
                    $el.off(dt.transition.endEvent).css('height', '');
                    if (success)
                        success();
                });
            });
